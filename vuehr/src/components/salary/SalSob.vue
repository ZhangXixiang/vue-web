      ...
      <el-header style="display: flex;justify-content: space-between;align-items: center;padding-left: 0px">
        <el-form :inline="true" :model="formInline" class="demo-form-inline">
          <el-form-item label="回答标题">
            <el-input v-model="formInline.name" placeholder="回答标题" size="mini"  width="30"></el-input>
          </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      loadSalaryCfg(){
        ...
        var _this = this;
        this.getRequest("/salary/sob/answer?page=" + this.currentPage + "&size=10" +"&name="+this.formInline.name+"&topic="+this.formInline.topic).then(resp=> {
          _this.tableLoading = false;
          if (resp && resp.status == 200) {
            var data = resp.data;
